// SlotMaster — Ana JS
// API anahtarı ve admin bilgileri BURADA YOK — backend/.env'de
"use strict";
...
// Oturum kontrolü - DOMContentLoaded'da çağrılır
async function checkSession(){
  try{
    const res=await API.get('/api/auth/me');
    if(res.loggedIn){
      window._currentUser=res;
      if(res.role==='admin'||res.role==='moderator'){
        // Admin erişimi var — panel bağlantısını göster
      }
    }
  }catch(e){console.warn('Session check failed');}
}

// Sponsor tıklamaları backend'e logla
function trackSpClick(id){API.post('/api/sponsors/'+id+'/click',{}).catch(()=>{});}
...
function doLogin(){
  ...
  e.style.display='none';closeM('login');toast('Hoş geldin, '+u+' ✓');
}
function doRegister(){
